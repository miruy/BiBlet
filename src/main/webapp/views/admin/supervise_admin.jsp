			...
			<div id="adminManagement">
				...
				<form class="flex flex-row justify-center" name="requestAdmSearch" method="post" action="/admin/supervise_admin">
						...
						<div class="relative w-96">
							...
							<input type="submit" id="admin_search" value="검색" class="text-white absolute right-2.5 bottom-2.5 top-2.5 text-sm font-medium text-gray-600 hover:text-white btn-secondary w-16 h-10 rounded-lg" />
						</div>
							...
							<tr class="hover">
								...
								<td>
									<c:if test="${admin.admEmail.length() >= 25}">
										<textarea rows="1" class="w-14 bg-white resize-x" disabled>${admin.admEmail}</textarea>
									</c:if>
									<c:if test="${admin.admEmail.length() < 25}">
										${admin.admEmail}
									</c:if>
								...
								<td>
									<fmt:parseDate value="${admin.admRegDate}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="parsedDateTime" type="both" />
									<fmt:formatDate pattern="yyyy/MM/dd HH:mm:ss" value="${ parsedDateTime }" />
								</td>
								<td>
									<c:if test="${admin.admAuthstatus == 1}">
										<span>인증</span>
									</c:if>
									<c:if test="${admin.admAuthstatus == 0}">
										<span>미인증</span>
									</c:if>
								</td>
